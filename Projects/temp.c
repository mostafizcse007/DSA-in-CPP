...
struct Student
{
    ...
    char password[MAX_ROLL_NUMBER_LENGTH];
};
...
void initializeUsers(struct User users[], struct Student students[], int studentCount) {
    strcpy(users[0].username, "admin");
    strcpy(users[0].password, "admin123");

    for (int i = 0; i < studentCount; i++) {
        strcpy(users[i + 1].username, students[i].name);
        strcpy(users[i + 1].password, students[i].rollNumber);
    }
}

int login(struct User users[], int userCount, struct Student students[], int studentCount) {
    char username[50], password[15];
    printf("Enter username (name for student, 'admin' for admin): ");
    scanf("%s", username);
    printf("Enter password (roll number for student, 'admin123' for admin): ");
    scanf("%s", password);
    if (strcmp(username, "admin") == 0 && strcmp(password, "admin123") == 0) {
        return 0;
    }
    for (int i = 0; i < studentCount; i++) {
        if (strcmp(students[i].name, username) == 0 && strcmp(students[i].rollNumber, password) == 0) {
            return -(i + 1);
        }
    }
...
int searchDelStudent(struct Student students[], int count, char rollNumber[])
{
    for (int i = 0; i < count; i++)
    {
        if (strcmp(students[i].rollNumber, rollNumber) == 0 && students[i].isDeleted)
...
void viewOwnData(struct Student students[], int studentCount, char *username)
{
    int found = 0;
    for (int i = 0; i < studentCount; i++)
    {
        if (strcmp(students[i].name, username) == 0 && !students[i].isDeleted)
        {
    ...
    if (!found)
    {
        printf("Your details could not be found.\n");
    }
}
void viewTopNStudents(struct Student students[], int count, int N)
{
    sortStudentsByAverage(students, count);
    printf("\nTop %d Students:\n", N);
...
void restoreStudent(struct Student *s)
{
    ...
    char confirmation;
    printf("Are you sure you want to restore this student (y/n)? ");
    scanf(" %c", &confirmation);
    if (confirmation == 'y' || confirmation == 'Y')
    {
        s->isDeleted = 0;
        printf("Student record restored.\n");
    }
    else
    {
...
int main()
{
    ...
    userCount = 1 + count;
    initializeUsers(users, students, count);

    printf("\n--- Login Panel ---\n");
    while (loggedInUserIndex == -1) {
        int result = login(users, userCount, students, count);
        
        if (result >= 0) {
            loggedInUserIndex = result;
            isAdmin = 1;
            printf("Admin logged in successfully!\n");
        } else if (result < 0) {
            loggedInUserIndex = -result - 1;
            isAdmin = 0;
    ...
    if (!isAdmin) {
        printf("Your details:\n");
        viewOwnData(students, count, users[loggedInUserIndex+1].username);
    }
        ...
        switch (choice)
        {
            ...
            case 2:
                ...
                if (isAdmin) {
                    ...
                    changePassword(&users[loggedInUserIndex+1]);
                }
                break;
