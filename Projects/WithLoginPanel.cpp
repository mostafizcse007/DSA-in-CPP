...
void initializeUsers(struct User users[], int *userCount)
{
    ...
    strcpy(users[1].password, "student1");
    users[1].isAdmin = 0;

    strcpy(users[2].username, "student2");
    strcpy(users[2].password, "student2");
    users[2].isAdmin = 0;
...
{
    sortStudentsByAverage(students, count); // Sorts students by average marks
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        {
            students[i].rank = rank++; // Assign rank and increment
        }
    }
}
...
void viewOwnData(struct Student students[], int studentCount, char *username)
{
    ...
    {
        // Check if the username matches the student's roll number
        if (strcmp(students[i].rollNumber, username) == 0 && !students[i].isDeleted)
        {
...
int main()
{
    ...
    isAdmin = users[loggedInUserIndex].isAdmin;

    if (!isAdmin)
    {
        printf("Logged in as student.\n");
        printf("Your details:\n");
        viewOwnData(students, count, users[loggedInUserIndex].username); // Updated to use the logged-in user's username
    }
    ...
    do
    {
        ...
        case 2:
            ...
            {
                viewOwnData(students, count, users[loggedInUserIndex].username); // Use logged-in student’s username
            }
            break;
        ...
        case 3:
            ...
            {
                changePassword(&users[loggedInUserIndex]); // Change password for the logged-in student
            }
            break;
